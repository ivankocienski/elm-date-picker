module Bits.DatePicker exposing (DateSelectorMsg, dateSelectWidget, selectDateFromDateSelector, updateDateSelector)

import Bits.Icons exposing (calendarIcon)
import Date
import Html exposing (Html, button, div, h6, span, table, tbody, td, text, th, thead, tr)
...
dateSelectWidget today maybeSelectorValue msgWrapper =
    let
        calendarDay =
            Maybe.withDefault today maybeSelectorValue

        start =
            Date.floor Date.Week (Date.floor Date.Month calendarDay)

        lastDay =
            Date.floor Date.Month (Date.add Date.Months 1 calendarDay)
        ...
        renderWeek startOfWeek =
            let
                endOfWeek =
                    Date.add Date.Weeks 1 startOfWeek
            in
            tr []
